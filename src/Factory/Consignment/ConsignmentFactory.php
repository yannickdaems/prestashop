...
namespace Gett\MyparcelNL\Factory\Consignment;
...
use Gett\MyparcelNL\OrderLabel;
use Gett\MyparcelNL\Service\Order\OrderTotalWeight;
use Gett\MyparcelNL\Service\ProductConfigurationProvider;
use Module;
...
class ConsignmentFactory
{
    ...
    private function MYPARCELNL_RECIPIENT_ONLY(AbstractConsignment $consignment)
    {
        if ($consignment instanceof PostNLConsignment) {
    ...
    private function MYPARCELNL_AGE_CHECK(AbstractConsignment $consignment)
    {
        return $consignment->setAgeCheck(true);
    }

    private function MYPARCELNL_PACKAGE_TYPE(AbstractConsignment $consignment)
    {
        return $consignment->setPackageType($this->request['packageType']);
    }

    private function MYPARCELNL_INSURANCE(AbstractConsignment $consignment)
    {
        $insuranceValue = 0;
    ...
    private function MYPARCELNL_RETURN_PACKAGE(AbstractConsignment $consignment)
    {
        return $consignment->setReturn(true);
    }

    private function MYPARCELNL_SIGNATURE_REQUIRED(AbstractConsignment $consignment)
    {
    ...
    private function MYPARCELNL_PACKAGE_FORMAT(AbstractConsignment $consignment)
    {
