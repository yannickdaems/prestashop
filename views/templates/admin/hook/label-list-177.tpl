<table class="table">
  ...
  <tr>
    ...
    <th><span class="title_box">{l s='Track & Trace' mod='myparcelnl'}</span></th>
    <th><span class="title_box">{l s='Status' mod='myparcelnl'}</span></th>
    <th><span class="title_box">{l s='Last update' mod='myparcelnl'}</span></th>
    <th></th>
  </tr>
  </thead>
  <tbody>
  <tr class="tr-empty-notice{if !empty($labelList)} hidden d-none{/if}">
    <td class="list-empty hidden-print" colspan="5">
      <div class="list-empty-msg text-center">
        ...
        <div>{l s='There are no shipments' mod='myparcelnl'}</div>
      </div>
    </td>
  </tr>
  {if !empty($labelList)}
      {foreach $labelList as $label}
        <tr
                ...
                data-return="{l s='Return - %s' mod='myparcelnl' sprintf=[$label.barcode]}"
                class="tr-label-item"
        >
          ...
          <td class="order-label-action text-right">
            <div class="btn-group" id="btn_group_action">
              ...
              >
                <span class="material-icons">local_printshop</span> {l s='Print' mod='myparcelnl'}
              </button>
              <button
                      type="button"
                      ...
                      aria-expanded="false"
              >
                <span class="sr-only">{l s='Toggle Dropdown' mod='myparcelnl'}</span>
              </button>
              <div class="dropdown-menu">
                <a href="#" class="order-label-action-refresh dropdown-item">
                  <span class="material-icons">refresh</span>
                  {l s='Refresh' mod='myparcelnl'}
                </a>
                {if !empty($label.ALLOW_RETURN_FORM)}
                  <a href="#" class="order-label-action-return dropdown-item{if !empty($label.return_disabled)} disabled{/if}">
                    ...
                    {l s='Create return label' mod='myparcelnl'}
                  </a>
                {/if}
