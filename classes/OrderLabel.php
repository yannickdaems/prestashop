...
use Db;
use DbQuery;
use Gett\MyparcelNL\Service\Tracktrace;
use Language;
use Mail;
...
class OrderLabel extends \ObjectModel
{
    ...
    public static function updateStatus($idShipment, $barcode, $statusCode, $date = null)
    {
        ...
        if ($statusCode === 14) {
            ...
            if ($myparcel->isNL()
                && \Configuration::get(\Gett\MyparcelNL\Constant::SENT_ORDER_STATE_FOR_DIGITAL_STAMPS_CONFIGURATION_NAME)) {
    ...
    public static function sendShippedNotification(int $idShipment)
    {
        ...
        $mailDir = false;
        if (file_exists(_PS_THEME_DIR_ . "modules/myparcelnl/mails/{$mailIso}/myparcel_{$mailType}_shipped.txt")
            && file_exists(
                _PS_THEME_DIR_ . "modules/myparcelnl/mails/{$mailIso}/myparcel_{$mailType}_shipped.html"
            )
        ) {
            $mailDir = _PS_THEME_DIR_ . 'modules/myparcelnl/mails/';
        ...
        } elseif (file_exists(_PS_THEME_DIR_ . "modules/myparcelnl/mails/{$mailIsoEn}/myparcel_{$mailType}_shipped.txt")
            && file_exists(
                _PS_THEME_DIR_ . "modules/myparcelnl/mails/{$mailIsoEn}/myparcel_{$mailType}_shipped.html"
            )
        ) {
            $mailDir = _PS_THEME_DIR_ . 'modules/myparcelnl/mails/';
    ...
    public static function getDataForLabelsCreate(array $orderIds)
    {
        ...
        $qb->innerJoin('myparcelnl_delivery_settings', 'ds', 'o.id_cart = ds.id_cart');
    ...
    public static function getOrderDeliveryOptions(int $id_order)
    {
        ...
        $qb->from('myparcelnl_delivery_settings', 'ds');
    ...
    public static function getOrdersLabels(array $orders_id)
    {
        ...
        $qb->from('myparcelnl_order_label', 'ol');
    ...
    public static function getOrderLabels(int $order_id, array $label_ids = [])
    {
        $sql = new DbQuery();
        $sql->select('*');
        $sql->from('myparcelnl_order_label');
    ...
    public static function getCustomsOrderProducts(int $id_order)
    {
        ...
        $qb->from('order_detail', 'od');
        $qb->leftJoin('myparcelnl_product_configuration', 'pc', 'od.product_id = pc.id_product');
        $qb->where('od.id_order = ' . $id_order);
